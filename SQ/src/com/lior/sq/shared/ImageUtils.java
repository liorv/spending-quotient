...
public class ImageUtils
{
  ...
  public static ImageData cropImage(Image image, double sx, double sy,
      double sw, double sh)
  {
  ...
  public static ImageData scaleAndCropImage(Image image, double scaleToRatio,
   ...
   * 
   * sx - the x coordinate of the upper-left corner of the source rectangle sy -
   * the y coordinate of the upper-left corner of the source rectangle sw - the
   * width of the source rectangle sh - the width of the source rectangle dx -
   * the x coordinate of the upper-left corner of the destination rectangle dy -
   * the y coordinate of the upper-left corner of the destination rectangle dw -
   * the width of the destination rectangle dh - the height of the destination
   * rectangle
   */
  public static ImageData scaleImage(Image image, double scaleToRatio) {

    // System.out.println("PanoTiler.scaleImag()e: scaleToRatio=" + scaleToRatio
    // + " width=" + width + " x height=" + height);

    Canvas canvasTmp = Canvas.createIfSupported();
    Context2d context = canvasTmp.getContext2d();

    double ch = (image.getHeight() * scaleToRatio) + 100; // 100 is offset so it
                                                          // doesn't throw
    double cw = (image.getWidth() * scaleToRatio) + 100;
    ...
    ImageData imageData = context.getImageData(0, 0, w, h); // this won't get
                                                            // the extra 100

    return imageData;
  }

  public static ImageData cropImage(ImageData imageData, double sx, double sy,
      double sw, double sh)
  {
    ...
    // get image data
    // imageData = context.getImageData(0, 0, imageData.getWidth(),
    // imageData.getHeight());
